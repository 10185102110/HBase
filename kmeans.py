...
def zc_read_csv():
    zc_dataframe = pd.read_csv("C:/Users/HP/Desktop/mobike/mobike/1/123.csv", sep=",")
    x = []
    
    for zc_index in zc_dataframe.index:
        zc_row = zc_dataframe.loc[zc_index]
        x.append([zc_row[6],zc_row[7]])
    return x

x = zc_read_csv()
y = tf.convert_to_tensor(x,name='y')
z = tf.to_double(y, name='z')
...
def kmeans(vectors, k_num):
    ...
    # 异常处理，防止后续数组下标越界,防止出现分类个数大于实际的点的个数
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            # 从现有的点集合中抽取出前 k_num 个数据作为默认的中心点,并且定义为 tf 的变量,
            # 用于后续的中心点的运算
            ...
            # cluster_assigns 存储的是每个点到 k_num 个中心点中的最小的一个距离
            cluster_assigns = []
            # 初始化 cluster_assigns
